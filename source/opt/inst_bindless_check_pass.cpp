...
uint32_t InstBindlessCheckPass::CloneOriginalReference(
    InstBindlessCheckPass::ref_analysis* ref, InstructionBuilder* builder) {
...
bool InstBindlessCheckPass::AnalyzeDescriptorReference(
    Instruction* ref_inst, InstBindlessCheckPass::ref_analysis* ref) {
...
uint32_t InstBindlessCheckPass::FindStride(uint32_t ty_id,
  ...
  get_decoration_mgr()->WhileEachDecoration(
      ty_id, stride_deco, [&stride](const Instruction& deco_inst) {
        stride = deco_inst.GetSingleWordInOperand(1u);
        return false;
      });
  return stride;
}
...
uint32_t InstBindlessCheckPass::GenLastByteIdx(
    InstBindlessCheckPass::ref_analysis* ref, InstructionBuilder* builder) {
  ...
  while (ac_in_idx < ac_inst->NumInOperands()) {
    ...
    switch (curr_ty_inst->opcode()) {
      ...
      case SpvOpTypeMatrix: {
        ...
        uint32_t arr_stride = 0xdeadbeef;
        get_decoration_mgr()->WhileEachDecoration(
            curr_ty_id, stride_deco,
            [&arr_stride](const Instruction& deco_inst) {
              arr_stride = deco_inst.GetSingleWordInOperand(2u);
              return false;
            });
...
void InstBindlessCheckPass::GenCheckCode(
    ...
    uint32_t length_id, uint32_t stage_idx,
    InstBindlessCheckPass::ref_analysis* ref,
