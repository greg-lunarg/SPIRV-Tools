...
uint32_t InstBindlessCheckPass::GenDebugReadLength(
  ...
  // The lengths for the runtime descriptor array at the end of each descriptor
  // set (if any) are at the beginning of the debug input buffer for the stage
  // and can be indexed with the descriptor set index.
  uint32_t desc_set_idx = var2desc_set_[image_id];
  return GenDebugDirectRead(stage_idx, desc_set_idx, builder);
}
...
void InstBindlessCheckPass::GenBindlessCheckCode(
  ...
  if (ptr_type_inst->opcode() == SpvOpTypeArray) {
    ...
    if (index_inst->opcode() == SpvOpConstant &&
      ...
      return;
  } else if (ptr_type_inst->opcode() != SpvOpTypeRuntimeArray) {
    return;
  }
  ...
  if (length_id == 0) {
    ...
    length_id = GenDebugReadLength(stage_idx, image_id, &builder);
  }
...
void InstBindlessCheckPass::InitializeInstBindlessCheck() {
  ...
  // If descriptor indexing extension enabled, create variable to descriptor
  // set mapping.
  if (ext_descriptor_indexing_defined_)
    for (auto& anno : get_module()->annotations())
      if (anno.opcode() == SpvOpDecorate && 
          anno.GetSingleWordInOperand(1u) == SpvDecorationDescriptorSet)
