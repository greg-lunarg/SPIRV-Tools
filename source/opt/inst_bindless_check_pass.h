...
class InstBindlessCheckPass : public InstrumentPass {
 ...
 private:
  ...
  // Use |error_id|, length_id and |stage_idx| (among other saved info) to
  // create debug output. Generate conditional branch to a valid or invalid
  // branch. Generate valid block which does original reference. Generate
  // invalid block which writes debug output. Generate merge block for valid
  // and invalid branches. Kill original reference.
  ...
  // Descriptor reference analysis components, both ids and instructions
  //
  // Image value
  uint32_t image_id_;
  ...
  uint32_t var_id_;
  Instruction* var_inst_;
  // Index into descriptor array
  uint32_t index_id_;
  // Original reference
  Instruction* ref_inst_;
};
