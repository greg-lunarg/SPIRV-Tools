...
class InstructionBuilder {
 public:
  ...
  Instruction* AddIdIdLiteralOp(uint32_t type_id, SpvOp opcode, uint32_t id0,
                                uint32_t id1, uint32_t uliteral) {
    // TODO(1841): Handle id overflow.
    std::unique_ptr<Instruction> newBinOp(new Instruction(
      GetContext(), opcode, type_id, GetContext()->TakeNextId(),
      { { spv_operand_type_t::SPV_OPERAND_TYPE_ID,{ id0 } },
        { spv_operand_type_t::SPV_OPERAND_TYPE_ID,{ id1 } },
