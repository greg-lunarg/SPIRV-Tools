...
// This class/pass is designed to support the physical buffer address
...
class InstBuffAddrCheckPass : public InstrumentPass {
 ...
 private:
   // Return byte length of |type_id|. Must be int, float, vector, matrix or
   // physical pointer.
   ...
   // Return id for search and test function. Generate it if not already.
   uint32_t GetSearchAndTestFuncId();

   // Generate code into |builder| to do search for buffer used by |ref_inst|
   // and test that all bytes of reference are within the buffer. Returns
   // id of boolean value which is true if search and test is successful,
   // false otherwise
  ...
  // GenPhysBuffAddrCheckCode generates code that checks that all bytes that
