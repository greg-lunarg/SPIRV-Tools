...
class InstBuffAddrCheckPass : public InstrumentPass {
 public:
  ...
  InstBuffAddrCheckPass(uint32_t desc_set, uint32_t shader_id,
                        uint32_t version)
      : InstrumentPass(desc_set, shader_id, kInstValidationIdBuffAddr, version) {}
 ...
 private:
  ...
  //
  void GenBuffAddrCheckCode(BasicBlock::iterator ref_inst_itr,
                          UptrVectorIterator<BasicBlock> ref_block_itr,
                          uint32_t stage_idx,
                          std::vector<std::unique_ptr<BasicBlock>>* new_blocks);

  // Analyze reference |ref_inst| and save components into |ref|.
  // Return true if |ref_inst| is a buffer address reference, false otherwise.
  bool AnalyzeBuffAddrReference(Instruction* ref_inst);

  // Clone original reference |ref_inst| into |builder|.
  ...
  // utilizing |ref_inst|, |error_id| and |stage_idx|. Generate merge block
  // for valid and invalid branches. Kill original reference.
