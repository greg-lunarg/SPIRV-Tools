...
bool MergeGenericAddendSub(uint32_t addend, uint32_t sub, IRContext* context,
                           Instruction* inst) {
  analysis::DefUseManager* def_use_mgr = context->get_def_use_mgr();
...
FoldingRule MergeGenericAddSubArithmetic() {
  return [](IRContext* context, Instruction* inst,
    ...
    if (MergeGenericAddendSub(add_op0, add_op1, context, inst)) return true;
    return MergeGenericAddendSub(add_op1, add_op0, context, inst);
  };
}
...
FoldingRule FactorAddMuls() {
  return [](IRContext* context, Instruction* inst,
    ...
    if (add_op0_inst->opcode() == SpvOpFSub &&
        (!add_op0_inst->IsFloatingPointFoldingAllowed() ||
         !add_op1_inst->IsFloatingPointFoldingAllowed()))
      return false;

    uint32_t mul_op0_0 = add_op0_inst->GetSingleWordInOperand(0);
    uint32_t mul_op0_1 = add_op0_inst->GetSingleWordInOperand(1);
    uint32_t mul_op1_0 = add_op1_inst->GetSingleWordInOperand(0);
    uint32_t mul_op1_1 = add_op1_inst->GetSingleWordInOperand(1);

    // Attempt all combinations of multiplicands for match
    if (FactorAddMulsOpnds(mul_op0_0, mul_op0_1, mul_op1_0, mul_op1_1, context,
                           inst))
      return true;
    if (FactorAddMulsOpnds(mul_op0_0, mul_op0_1, mul_op1_1, mul_op1_0, context,
                           inst))
      return true;
    if (FactorAddMulsOpnds(mul_op0_1, mul_op0_0, mul_op1_0, mul_op1_1, context,
                           inst))
      return true;
    return FactorAddMulsOpnds(mul_op0_1, mul_op0_0, mul_op1_1, mul_op1_0,
                              context, inst);
  };
}
