...
uint32_t InstrumentPass::GenDebugDirectRead(
    uint32_t stage_idx, uint32_t idx, InstructionBuilder* builder) {
  uint32_t input_buf_id = GetInputBufferId(stage_idx);
  uint32_t buf_uint_ptr_id = GetBufferUintPtrId();
  Instruction* ibuf_ac_inst =
      builder->AddTernaryOp(buf_uint_ptr_id, SpvOpAccessChain, input_buf_id,
                            builder->GetUintConstantId(kDebugInputDataOffset),
                            builder->GetUintConstantId(idx));
  Instruction* load_inst =
...
uint32_t InstrumentPass::GetInputBufferBinding(uint32_t stage_idx) {
  switch (validation_id_) {
    case kInstValidationIdBindless:
      return kDebugInputBindingOffsetBindless + stage_idx;
    default:
      assert(false && "unexpected validation id");
  }
  return 0;
...
uint32_t InstrumentPass::GetInputBufferId(uint32_t stage_idx) {
  if (input_buffer_id_ == 0) {
    ...
    analysis::Type* reg_uint_rarr_ty =
      type_mgr->GetRegisteredType(&uint_rarr_ty);
    analysis::Struct ibuf_ty({ reg_uint_rarr_ty });
    analysis::Type* reg_ibuf_ty = type_mgr->GetRegisteredType(&ibuf_ty);
    uint32_t ibufTyId = type_mgr->GetTypeInstruction(reg_ibuf_ty);
    ...
    deco_mgr->AddDecorationVal(input_buffer_id_, SpvDecorationBinding,
        GetInputBufferBinding(stage_idx));
    AddStorageBufferExt();
  }
