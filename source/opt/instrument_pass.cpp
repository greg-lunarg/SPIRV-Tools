...
void InstrumentPass::GenStageStreamWriteCode(uint32_t stage_idx,
  ...
  switch (stage_idx) {
    case SpvExecutionModelVertex: {
      ...
      GenBuiltinOutputCode(context()->GetBuiltinInputVarId(SpvBuiltInVertexIndex),
                           kInstVertOutVertexIndex, base_offset_id, builder);
      GenBuiltinOutputCode(context()->GetBuiltinInputVarId(SpvBuiltInInstanceIndex),
    ...
    case SpvExecutionModelGLCompute: {
      ...
      if (version_ == 1) {
        ...
        Instruction* x_or_y_or_z_inst = builder->AddBinaryOp(
            GetUintId(), SpvOpBitwiseOr, x_or_y_inst->result_id(),
            z_inst->result_id());
        ...
        GenDebugOutputFieldCode(base_offset_id,
            kInstCompOutGlobalInvocationIdX, x_inst->result_id(), builder);
        GenDebugOutputFieldCode(base_offset_id,
            kInstCompOutGlobalInvocationIdY, y_inst->result_id(), builder);
        GenDebugOutputFieldCode(base_offset_id,
            kInstCompOutGlobalInvocationIdZ, z_inst->result_id(), builder);
      }
    } break;
    case SpvExecutionModelGeometry: {
      // Load and store PrimitiveId and InvocationId.
      GenBuiltinOutputCode(context()->GetBuiltinInputVarId(SpvBuiltInPrimitiveId),
                           kInstGeomOutPrimitiveId, base_offset_id, builder);
      GenBuiltinOutputCode(context()->GetBuiltinInputVarId(SpvBuiltInInvocationId),
    ...
    case SpvExecutionModelTessellationEvaluation: {
      if (version_ == 1) {
        ...
        Instruction* v_inst = builder->AddIdLiteralOp(
            GetUintId(), SpvOpCompositeExtract, load_id, 1);
        GenDebugOutputFieldCode(base_offset_id,
            kInstTessEvalOutTessCoordU, u_inst->result_id(), builder);
        GenDebugOutputFieldCode(base_offset_id,
            kInstTessEvalOutTessCoordV, v_inst->result_id(), builder);
      }
    } break;
...
uint32_t InstrumentPass::GetStreamWriteFunctionId(uint32_t stage_idx,
  ...
  if (output_func_id_ == 0) {
    ...
    uint32_t val_spec_offset = (version_ == 1) ? kInstStageOutCnt : kInst2StageOutCnt;
