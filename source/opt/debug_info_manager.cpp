...
uint32_t DebugInfoManager::CreateDebugInlinedAt(const Instruction* line,
  ...
  if (line == nullptr) {
    ...
    if (line_number_type == spv_operand_type_t::SPV_OPERAND_TYPE_ID) {
      uint32_t line_id = context()->TakeNextId();

      std::unique_ptr<Instruction> line_const(new Instruction(
          context(), SpvOpConstant, context()->get_type_mgr()->GetUIntTypeId(),
          line_id,
          {
      ...
      if (context()->AreAnalysesValid(IRContext::Analysis::kAnalysisDefUse))
        context()->get_def_use_mgr()->AnalyzeInstDefUse(line_const.get());
      context()->module()->AddGlobalValue(std::move(line_const));
...
Instruction* DebugInfoManager::GetDebugOperationWithDeref() {
  ...
  if (context()->get_feature_mgr()->GetExtInstImportId_OpenCL100DebugInfo()) {
    ...
    uint32_t deref_id = context()->TakeNextId();

    std::unique_ptr<Instruction> deref_const(new Instruction(
        context(), SpvOpConstant, context()->get_type_mgr()->GetUIntTypeId(),
        deref_id,
        {
            {spv_operand_type_t::SPV_OPERAND_TYPE_TYPED_LITERAL_NUMBER,
    ...
    context()->module()->AddGlobalValue(std::move(deref_const));
...
void DebugInfoManager::ClearDebugInfo(Instruction* instr) {
  ...
  if (debug_info_none_inst_ == instr) {
    ...
    for (auto dbg_instr_itr = context()->module()->ext_inst_debuginfo_begin();
         ...
         ++dbg_instr_itr) {
      if (instr != &*dbg_instr_itr &&
          dbg_instr_itr->GetCommonDebugOpcode() ==
