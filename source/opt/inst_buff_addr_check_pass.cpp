...
uint32_t InstBuffAddrCheckPass::CloneOriginalReference(
  ...
  uid2offset_[added_inst->unique_id()] =
      uid2offset_[ref_inst->unique_id()];
  if (new_ref_id != 0)
...
bool InstBuffAddrCheckPass::AnalyzeBuffAddrReference(Instruction* ref_inst) {
  ...
  if (ptr_inst->opcode() != SpvOpAccessChain)
    return false;
  uint32_t ptr_ty_id = ptr_inst->type_id();
  Instruction* ptr_ty_inst = du_mgr->GetDef(ptr_ty_id);
  if (ptr_ty_inst->GetSingleWordInOperand(0) != SpvStorageClassPhysicalStorageBufferEXT)
    return false;
  return true;
}
...
void InstBuffAddrCheckPass::GenCheckCode(
  ...
  Instruction* lo_uptr_inst = builder.AddUnaryOp(GetUintId(), SpvOpUConvert,
      ref_uptr_id);
  Instruction* rshift_uptr_inst = builder.AddBinaryOp(GetUint64Id(),
      SpvOpShiftRightLogical, ref_uptr_id, builder.GetUintConstantId(32));
  Instruction* hi_uptr_inst = builder.AddUnaryOp(GetUintId(), SpvOpUConvert,
      rshift_uptr_inst->result_id());
  GenDebugStreamWrite(uid2offset_[ref_inst->unique_id()], stage_idx,
                      {error_id, lo_uptr_inst->result_id(),
                       hi_uptr_inst->result_id() },
                      &builder);
  ...
  if (new_ref_id != 0) {
    Instruction* phi_inst = builder.AddPhi(
        ref_type_id, {new_ref_id, valid_blk_id, builder.GetNullId(ref_type_id),
                      last_invalid_blk_id});
    context()->ReplaceAllUsesWith(ref_inst->result_id(),
                                  phi_inst->result_id());
  }
  new_blocks->push_back(std::move(new_blk_ptr));
...
void InstBuffAddrCheckPass::AddParam(
    uint32_t type_id,
...
#define CUTOFF 0
  if (search_test_func_id_ == 0) {
    ...
    std::vector<const analysis::Type*> param_types = { type_mgr->GetType(GetUint64Id()), type_mgr->GetType(GetUintId()) };
    ...
    std::unique_ptr<Instruction> func_inst(new Instruction(
        get_module()->context(), SpvOpFunction, GetBoolId(), search_test_func_id_,
        { { spv_operand_type_t::SPV_OPERAND_TYPE_LITERAL_INTEGER,
        { SpvFunctionControlMaskNone } },
        { spv_operand_type_t::SPV_OPERAND_TYPE_ID,
        { type_mgr->GetTypeInstruction(reg_func_ty) } } }));
    get_def_use_mgr()->AnalyzeInstDefUse(&*func_inst);
    std::unique_ptr<Function> input_func = MakeUnique<Function>(std::move(func_inst));
    std::vector<uint32_t> param_vec;
    ...
    // If loaded address greater than ref_ptr arg, break, else branch back to loop
    // header
    ...
    Instruction* offset_inst = builder.AddBinaryOp(
        ibuf_type_id, SpvOpISub, param_vec[0],
    ...
    Instruction* ref_end_inst = builder.AddBinaryOp(
        ibuf_type_id, SpvOpIAdd, offset_inst->result_id(),
        ref_len_64_inst->result_id());
    // Load starting index of lengths in input buffer and convert to uint32
    Instruction* len_start_ac_inst = builder.AddTernaryOp(
        ibuf_ptr_id, SpvOpAccessChain, ibuf_id,
        builder.GetUintConstantId(kDebugInputDataOffset),
        builder.GetUintConstantId(0u));
    Instruction* len_start_load_inst = builder.AddUnaryOp(ibuf_type_id,
    ...
    Instruction* len_ac_inst = builder.AddTernaryOp(
        ibuf_ptr_id, SpvOpAccessChain, ibuf_id,
        builder.GetUintConstantId(kDebugInputDataOffset),
        len_idx_inst->result_id());
    Instruction* len_load_inst = builder.AddUnaryOp(ibuf_type_id, SpvOpLoad,
        len_ac_inst->result_id());
    // Test if reference end within candidate buffer length
    Instruction* len_test_inst = builder.AddBinaryOp(GetBoolId(),
        SpvOpULessThanEqual, ref_end_inst->result_id(),
        len_load_inst->result_id());
    // Return test result
    (void)builder.AddInstruction(MakeUnique<Instruction>(
        context(), SpvOpReturnValue, 0, 0,
        std::initializer_list<Operand>{ {SPV_OPERAND_TYPE_ID, { len_test_inst->result_id() }}}));
    // Close block
...
uint32_t InstBuffAddrCheckPass::GenSearchAndTest(
  ...
  *ref_uptr_id = ref_uptr_inst->result_id();
  ...
  const std::vector<uint32_t> args = { GetSearchAndTestFuncId(), *ref_uptr_id, ref_len_id };
  Instruction* call_inst = builder->AddNaryOp(GetBoolId(), SpvOpFunctionCall, args);
...
void InstBuffAddrCheckPass::GenBuffAddrCheckCode(
  ...
  if (!AnalyzeBuffAddrReference(ref_inst)) return;
  ...
  GenCheckCode(valid_id, error_id, ref_uptr_id, stage_idx, ref_inst, new_blocks);
...
Pass::Status InstBuffAddrCheckPass::Process() {
  if (!get_feature_mgr()->HasCapability(SpvCapabilityPhysicalStorageBufferAddressesEXT))
